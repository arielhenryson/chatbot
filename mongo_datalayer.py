...
class MongoDBDataLayer(BaseDataLayer):
    ...
    async def get_thread(self, thread_id: str) -> Optional[ThreadDict]:
        ...
        if thread:
            thread["userIdentifier"] = thread.get("user_id")
            thread["_id"] = str(thread.pop("_id"))
    ...
    async def create_step(self, step_dict: StepDict):
        if step_dict.get('push', False) is not True:
            return  # Ignore this step_dict if 'push' is not True
    ...
    # Query MongoDB to find the element within the specified thread
        thread = await self.threads.find_one({"id": thread_id, "elements.id": element_id})
        
        if not thread:
            return None

        # Find the specific element inside the 'elements' array
        element = next((e for e in thread["elements"] if e["id"] == element_id), None)

        if not element:
            return None
        
        # Extract the necessary metadata (assumes it's in the 'metadata' field)
        ...
        return ElementDict(
            ...
            mime=str(element["mime"]),
            objectKey=str(element["objectKey"]),
            forId=str(element["stepId"]),
